...
const SearchSelectLocation = ({ route, navigation }) => {
    ...
    const changePlace = (which) => {
        if (which === 0) {
            isPlaceTeacherChecked ? setPlaceTeacherChecked(false) : setPlaceTeacherChecked(true)
        } else {
            isPlaceStudentChecked ? setPlaceStudentChecked(false) : setPlaceStudentChecked(true)
        }
    }
    ...
    const goNextScreen = (localization, radius, placeStudent, placeTeacher) => {
        ...
        else if (!isPlaceStudentChecked && !isPlaceTeacherChecked) {
            let toast = Toast.show('Wybierz miejsce odbywania się korepetycji', {
                duration: Toast.durations.LONG,
            });
        }
        else {
